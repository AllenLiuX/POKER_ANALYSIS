...
- 错题回顾可逐条展开（含翻前逐手 AI 讲解，重放代表手牌调 `/api/trainer/coach`）。
- AI 复盘报告：`POST /api/trainer/review` 把 attempts 聚合事实（含"过紧/过松/太被动/太激进"倾向分类）喂给 LLM，产出漏洞分析 + 训练建议；数字为权威事实，LLM 只解释。文本模型空返回自动回退 gpt-4o（`provider.text`）。
